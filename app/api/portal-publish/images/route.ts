...
export async function POST(request: Request) {
  ...
  const objektId = String(form.get("objekt_id") ?? "").trim();
  ...
  if (error) {
    console.error("[portal-images] upload failed:", error.message);
    return NextResponse.json(
  ...
  const { data } = supabase.storage.from("objekt-bilder").getPublicUrl(path);
