      ...
      <div className="space-y-3 p-3.5">
        <div>
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="flex size-8 shrink-0 items-center justify-center rounded-[10px] bg-[var(--accent-light)] text-[var(--accent)]">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            ...
            <span className="text-[10px] text-[var(--text-muted)]">{summary.quelle}</span>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-1.5 rounded-[12px] border border-[var(--border)] bg-[var(--bg-elevated)] px-2 py-2">
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="text-center">
      <p className="text-[14px] font-semibold tabular-nums text-[var(--text-primary)]">{value}</p>
      <p className="mt-0.5 text-[9px] leading-tight text-[var(--text-secondary)]">{label}</p>
    </div>
  );
}
