...
/** Generische Intent-Regeln für Branchen-Skills ohne spezifische KI-Logik. */
export const GENERIC_INDUSTRY_INTENT_RULES: IntentRule[] = [
  {
    intent: "Terminwunsch",
    keywords: [
      "terminanfrage",
  ...
  {
    intent: "Neue Anfrage",
    keywords: ["reklamation", "beschwerde", "unzufrieden", "reklamieren"],
  },
  {
    intent: "Neue Anfrage",
    keywords: ["anfrage", "information", "info", "preisanfrage", "preis"],
  },
  {
    intent: "Rückruf",
    keywords: ["rückruf", "rueckruf", "zurückrufen", "anrufen"],
  },
];
 ...
 */
export const SKILL_INTENT_RULES: Record<HelpySkill, IntentRule[]> = buildSkillRecord(
  {
    "real-estate": [
  ...
  GENERIC_INDUSTRY_INTENT_RULES
);

export function getSkillIntentRules(skill: HelpySkill): IntentRule[] {
  return SKILL_INTENT_RULES[skill] ?? GENERIC_INDUSTRY_INTENT_RULES;
}

export function helpySkillToBrainSkill(skill: HelpySkill): BrainV3Skill {
  switch (skill) {
    case "construction":
