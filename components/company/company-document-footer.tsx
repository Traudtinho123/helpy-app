...
export function CompanyDocumentFooter({
  ...
  return (
    <footer className="mt-10 border-t border-[var(--border)] pt-8">
      {closingText && (
        <div className="whitespace-pre-line text-[12px] leading-[1.75] text-[var(--text-secondary)]">
          {closingText}
        </div>
      )}
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="text-left text-[10px] text-[var(--text-muted)] sm:text-right">
            <p>{profile.taxId}</p>
        ...
        {profile.footer && (
          <p className="mt-4 whitespace-pre-line border-t border-[#F1F5F9] pt-4 text-[10px] leading-relaxed text-[var(--text-muted)]">
            {profile.footer}
          </p>
        )}
      </div>
